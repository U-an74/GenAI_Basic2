...
- 촬영 스타일: POV
- 내레이션(또는 카피): 없음
- 전환 효과: 빠른 컷 → SCENE. 5로 연결(SCENE.4 효과음이 연결 고리 역할)
- 사용 도구
    - 이미지 생성: **Leonardo AI(Nano banana)**
    - 비디오 변환: **adobe firefly(Veo 3.1 fast)**
    - 효과음 생성: **ElevenLabs**
...
- 내레이션(또는 카피): When Instinct Awakens, Magma Runs
    ...
    → 광고 분위기에 적합한 보이스 옵션(ElevenLabs의 **Edward : a deep, low, dark, seductive, strong british**)를 선택하여 생성
    
- 전환 효과: 빠른 컷 → 마지막 구간(1s) 브랜드 인지 장치(로고)로 연결(SCENE.5 내레이션이 연결 고리 역할)
- 사용 도구
...
→ 기존의 스토리 대신, 무게감 있는 차량 외관 reveal씬과 폭발적인 주행씬만으로 구성하여 담백하게 표현  

> (2) 메시지 재구성안: [본능이 깨어날 때, 마그마가 달린다] → [침묵이 폭발한다] 

→ 브랜드 usp(정숙함 이면의 퍼포먼스)를 다이렉트하게 전달
...
뒤에서 블러드오렌지 빛 용암이 쫓아감, 극적인 역광에 실루엣으로 표현, 모션 블러, 시네마틱, 초실사 8K 

[IMAGE TO VIDEO GENERATION PROMPT for Hailuo AI]
화산재로 뒤덮인 황야를 공황 상태로 달아나는 남자를 트래킹 샷, 어두운 하늘에서 화산재가 비처럼 내리고,
뒤에서 용암이 그를 쫓는다. 극적인 역광에 실루엣으로 표현, 모션 블러, 시네마틱, 초실사 8K
```
...
```jsx
**[IMAGE GENERATION PROMPT for Imagen 4 Standard]**
**## Concept**
어두운 재킷을 입은 남성이 거대한 화산 폭발을 피해 필사적으로 달리는 인물 액션 스틸컷
초현실적 영화적 재난 영화 미학, 와이드스크린
...
**[IMAGE TO VIDEO GENERATION PROMPT for Veo 3.1 Fast]**
**## Concept**
...
**## Effects**
**다리에 약간의 모션 블러를 넣어, 엄청난 속도로 달리고 있다는 느낌**을 준다.
강렬한 핏빛 오렌지색의 주변광이 남자의 옆쪽에서 얼굴과 몸을 비춘다.
**역광은 사용하지 않고, 얼굴과 옷이 명확하게 보일 것.** |
```
...
(2) 뒤에서 용암이 쫓아감 → 카메라 앵글 부적합 

: 완벽한 90도 측면 프로필로 표현, 프레임을 가로질러 수평으로 달리기 조건 추가

(3) 극적인 역광, 실루엣 표현 → 완전 검은 실루엣 유발 + 역광 강조로 인물이 보이지 않음. 

: 포토리얼리스틱 인체 해부학, 사실적인 피부 질감/얼굴 특징/의상을 구체적으로 기술

상기 3가지 개선하는 내용을 추가하고, [컨셉 / 캐릭터 구성 / 모션 / 배경 효과 / 카메라 앵글] 5가지 항목으로 
분류하여 기존 프롬프트를 수정 및 구체화함. 

<br>

**2. 비디오 재생성**

(1) 빠른 카메라 움직임 → 속도감 표현 부족

: 극도의 긴박함과 목숨 걸고 살아남기 위한 절박함 표현 추가 및 다리 모션 블러 효과 추가

(2) 트래킹 샷 팔로잉 → 카메라 촬영 방향 불명확

: 정확한 측면 프로필을 유지하는 트래킹 숏으로 촬영 방향 지시 추가

1.에서 수정한 레퍼런스 이미지를 새로 제시하고, 상기 2가지 개선포인트 추가하여 기존 프롬프트를 수정함. 

또한 [컨셉 / 촬영기법 / 장면세부묘사 / 효과] 4개 항목으로 분류하여 구체화 시킴.
...
**수정 전**
```jsx
[IMAGE TO VIDEO GENERATION for Hailuo AI]
고성능 전기차 내부를 조감한 샷,
...
**수정 후**
```jsx
[IMAGE TO VIDEO GENERATION for Veo 3.1 Fast]
**## Concept**
...
: 핸들에 브랜드 로고 또는 엠블럼 없음 조건 추가

(2) 엑셀 밟는 동작과 핸들링의 강도 약함 → 강도 표현 부족

: 온몸의 긴장감으로 힘껏 핸들링 할 것 지시, 페달이 바닥 매트까지 완전히 눌린 상태 조건을 추가

(3) 탑승 장면 생략 → 프롬프트상 누락

: 촬영 시작점을 문이 쾅 닫히는 순간의 운전석 시점으로 고정

상기 3가지 개선 포인트 추가하여, 기존 프롬프트를 수정함. 

또한, [컨셉 / 촬영기법 / 장면세부묘사 / 효과] 4개 항목으로 분류하여 구체화
...
→ 주 도구 사용이 어려운 상황 발생 시, 아래 표의 대체 도구를 순차적으로 시도하여 작업 연속성을 확보한다.

> 도구 교체 우선순위 기준: ① 크레딧 소진 / ② 품질 미달 / ③ 기능 제한 / ④ 대기열 과부하

→ 단순 선호가 아닌 작업 연속성과 결과물 품질 유지를 최우선으로 판단하여 설정함.
...
| 구분 | 도구명 | 선택 이유/목적 |
|------|--------|----------------|
| **주 도구** | **Leonardo AI** (Nano banana, Lucid origin) | 매일 무료 크레딧 충전, 일관성 유지 강점, 고퀄리티 고속 생성, auto 모드(인공 지능으로 제휴 AI 모델 중 프롬프트 구현에 가장 적절한 모델을 자동 선택) 활용 가능 |
| 대체 1 | Bing Image Creator (MAI-Image-2.5-Flash) | 크레딧 소진 후에도 사용 가능(단, 저속), 높은 프롬프트 이해력 강점으로 원하는 장면 구현 가능 |
...
| 구분 | 도구명 | 선택 이유/목적 |
|------|--------|----------------|
| **주 도구** | **Hailuo AI / MINIMAX H3** | 무료 크레딧 대량 제공, 영상과 어울리는 배경음악 동시 생성(별도 프롬프트 불필요), 고해상도 지원, 짧은 클립/모션 표현 강점 |
| 대체 1 | Adobe Firefly (Veo 3.1 Fast) | 제휴 AI 모델 3가지 (Ray3.14, Kling 3.0, Veo3.1 Fast) 무료 선택 가능, 일일 비디오 2회 생성 무료, 고해상도 지원 |

> [Kling] 가입시 무료 크레딧 일부 제공되나, 대기열 과부하로 사실상 무료 생성은 불가했음.
...
- 비디오 코덱: H.264
- 오디오 코덱: AAC
- 직촬 및 유료 스톡: 미사용(본 영상의 모든 소스는 AI 생성/산출물임.)
